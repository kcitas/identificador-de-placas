...
class PlateCropper:
    ...
    def crop(self, image: np.ndarray, bbox: BBox) -> tuple[np.ndarray, int, int]:
        """Returns the crop plus its (x, y) origin in the source image, so a
        box found inside the crop (e.g. by OCR) can be translated back."""
        h, w = image.shape[:2]
    ...
    def upscale_for_ocr(self, crop: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        Returns the scale factor applied, so a box found in the result can be
        divided back down to the pre-upscale crop's coordinates.
        """
        h, w = crop.shape[:2]
        if h >= TARGET_HEIGHT:
            return crop, 1.0
        ...
        return cv2.addWeighted(upscaled, 1.5, blurred, -0.5, 0), scale
